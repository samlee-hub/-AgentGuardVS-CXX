...
For profile-based non-VS projects, invoke the same Skill against a project root:

```text
@AgentGuardVS-CXX Use AgentGuardVS-CXX to safely modify this project with profile-based verification.
```
...
4. Confirm the Skill runs analyze first, edits only `allowed_files`, then runs verification and review. Visual Studio C++ tasks may use bundled wrapper scripts; other project types should use `AgentGuardVS.exe analyze --project`, `verify --project`, and `review`.
