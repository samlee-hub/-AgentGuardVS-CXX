...
AgentGuardVS-CXX is designed to reduce uncontrolled AI edits in profiled software projects. It provides process boundaries and audit artifacts, not a guarantee that an LLM will always make correct decisions.
...
- Build or command verification success does not prove business correctness.
...
- Visual Studio C++ with MSBuild remains the most mature end-to-end path.
- Non-VS projects rely on `ProjectProfile`, `SourceFilePolicy`, and configured verification commands.
