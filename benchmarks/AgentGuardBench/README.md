...
```bat
cd /d <external-path>\AgentGuardBench
...
The current first-version benchmark is based on one real project copy, `<external-path>\frame`. Although the repository count is limited, the task set covers multiple modules and validation dimensions that reflect common coding-agent failure modes in real engineering work.
