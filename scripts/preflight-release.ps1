...
$pathMatches = @(Test-Pattern -Files $files -Pattern '([A-Za-z]:\\Users\\[^\\]+|[A-Za-z]:/Users/[^/]+|D:\\testrepo|D:/testrepo)' |
    Where-Object {
        $_ -ne (Join-Path $repoRoot "scripts\preflight-release.ps1") -and
        $_ -ne (Join-Path $repoRoot "scripts\make-release-tree.ps1")
    })
