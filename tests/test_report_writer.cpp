...
TEST(ReportWriterTest, WritesMarkdownAndJsonWithAuditFields)
{
    ...
    request.audit = {
        {"project_kind", "python"},
        {"languages", nlohmann::json::array({"python"})},
        {"profile_source", "custom"},
        {"indexed_files_count", 4},
        {"skipped_files_count", 2},
        {"verify_commands", nlohmann::json::array({{{"name", "unit"}}})},
        {"verify_commands_count", 1},
        {"verify_result", nullptr},
        {"modified_files", nlohmann::json::array({"src/Summon.cpp"})},
        {"forbidden_write_attempts", nlohmann::json::array()}
    };
...

    {
        ...
        EXPECT_EQ(json_value.at("audit").at("project_kind").get<std::string>(), "python");
        EXPECT_EQ(json_value.at("audit").at("profile_source").get<std::string>(), "custom");
        EXPECT_EQ(json_value.at("audit").at("verify_commands").size(), 1U);
    }
