...
11. Do not blindly retry after analyze, verify, or review failure. Read the JSON error, prompt, raw response, build log, and report artifacts first.
...
- Use `--no-cache` through wrapper scripts when validating provider behavior or investigating stale model output.
