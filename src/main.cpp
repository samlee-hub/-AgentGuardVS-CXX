...
nlohmann::json BuildAuditJson(
    const agentguard::ProjectProfile& profile,
    const agentguard::SourceIndexResult& index_result,
    const nlohmann::json& verify_result = nullptr,
    const std::vector<std::string>& modified_files = {},
    const std::vector<std::string>& forbidden_write_attempts = {})
{
    std::vector<std::string> languages;
    for (const auto language : profile.languages)
    {
        languages.push_back(agentguard::LanguageKindToString(language));
    }
    return nlohmann::json{
        {"project_kind", agentguard::ProjectKindToString(profile.kind)},
        {"languages", languages},
        {"profile_source", profile.profile_source},
        {"indexed_files_count", index_result.files.size()},
        {"skipped_files_count", index_result.skipped_files},
        {"skipped_reasons", index_result.skipped_reasons},
        {"verify_commands_count", profile.verify_commands.size()},
...
int RunDetectCommand(const DetectOptions& options)
{
    try
    {
        ...
        else
        {
            ...
            std::cout << "Verify commands: " << profile.verify_commands.size() << "\n";
        }
        return 0;
    }
    catch (const std::exception& exception)
...
int RunVerifyProfileCommand(const VerifyProfileOptions& options)
{
    try
    {
        ...
        const auto index_result =
            agentguard::IndexSourceFilesWithPolicy(profile.root, profile.source_policy);
        ...
        if (options.json)
        {
            std::cout << nlohmann::json{
                ...
                {"audit", BuildAuditJson(profile, index_result, verification)}
            }.dump() << "\n";
        }
        else
        {
    ...
    catch (const std::exception& exception)
    {
        if (options.json)
        {
            return EmitJsonError(
                ...
                exception.what(),
                "Check agentguard.json verify_commands and executable paths.");
        }
        std::cerr << exception.what() << "\n";
        return 1;
    }
...
int RunDoctorCommand(const DoctorOptions& options)
{
    try
    {
        const auto profile = agentguard::DetectProjectProfile(options.project);
        const auto index_result =
            agentguard::IndexSourceFilesWithPolicy(profile.root, profile.source_policy);
        const nlohmann::json audit = BuildAuditJson(profile, index_result);
        const bool ok = profile.kind != agentguard::ProjectKind::Unknown && !index_result.files.empty();
        ...
        if (options.json)
        {
            std::cout << nlohmann::json{
                ...
                {"profile", profile},
                {"audit", audit},
                {"warnings", ok ? nlohmann::json::array() : nlohmann::json::array({
                    "Project kind is unknown or no source/config files were indexed."
                })}
            }.dump() << "\n";
        }
        else
        {
            std::cout << "Doctor: " << (ok ? "ok" : "warning") << "\n";
            std::cout << "Project kind: " << agentguard::ProjectKindToString(profile.kind) << "\n";
            std::cout << "Indexed files: " << index_result.files.size() << "\n";
            std::cout << "Verify commands: " << profile.verify_commands.size() << "\n";
        }
        return ok ? 0 : 1;
    }
    catch (const std::exception& exception)
    {
        if (options.json)
        {
            return EmitJsonError(
                ...
                "PROJECT_DETECT_FAILED",
                exception.what(),
                "Check that --project points to an existing project directory and agentguard.json is valid.");
        }
        std::cerr << exception.what() << "\n";
        return 1;
    }
...
int RunAnalyzeCommand(const AnalyzeOptions& options)
{
    try
    {
        ...
        const bool has_solution = !options.solution.empty();
        if (!has_project && !has_solution)
        {
            if (options.json)
            {
                return EmitJsonError(
                    "analyze",
                    "PROJECT_DETECT_FAILED",
                    "analyze requires --project <root> or --solution <file>.",
                    "Prefer --project for multi-language projects; keep --solution for legacy Visual Studio C++ workflows.");
            }
            std::cerr << "analyze requires --project <root> or --solution <file>.\n";
            return 2;
        }

        const fs::path target_path = has_project ? options.project : options.solution;
        const bool solution_is_sln =
            !has_project && fs::is_regular_file(options.solution) && options.solution.extension() == ".sln";
        const bool target_is_dir = fs::is_directory(target_path);
        if (!solution_is_sln && !target_is_dir)
        {
            if (options.json)
            {
                return EmitJsonError(
                    "analyze",
                    "SOLUTION_PARSE_FAILED",
                    "Target must point to an existing project directory or .sln file.",
                    "Pass --project <root> for multi-language projects or --solution <file.sln> for legacy Visual Studio C++.");
            }
            std::cerr << "Target must point to an existing project directory or .sln file.\n";
            return 2;
        }

        const fs::path source_root = solution_is_sln
            ? fs::absolute(options.solution).parent_path()
            : fs::absolute(target_path);
        const std::string task_id = MakeTaskId(target_path, options.task);
        ...
        const fs::path workspace_solution = solution_is_sln
            ? (workspace.repo_root / options.solution.filename())
            : workspace.repo_root;
        auto profile = agentguard::DetectProjectProfile(workspace.repo_root);

        agentguard::ProjectInfo project_info;
        project_info.solution_name = solution_is_sln
            ? workspace_solution.stem().string()
            : source_root.filename().string();
        project_info.solution_path = workspace_solution.string();
        if (solution_is_sln)
        {
        ...
        const auto index_result = agentguard::IndexSourceFilesWithPolicy(workspace.repo_root, profile.source_policy);
        const auto& source_files = index_result.files;
        std::vector<agentguard::SourceFileInfo> symbol_files;
        symbol_files.reserve(source_files.size());
        for (const auto& source_file : source_files)
        {
        ...
        report_request.audit = BuildAuditJson(profile, index_result);
        ...
        if (options.json)
        {
            std::cout << nlohmann::json{
                ...
                {"counts", {
                    ...
                    {"indexed", index_result.files.size()},
                    {"skipped", index_result.skipped_files}
                }}
            }.dump() << "\n";
        }
        else
        ...
        if (options.workspace.empty())
        {
            if (options.json)
            {
                return EmitJsonError(
                    "verify",
                    "WORKSPACE_CREATE_FAILED",
                    "verify requires --project <root> or --workspace <runs task root>.",
                    "Prefer --project for multi-language verification; keep --workspace for legacy MSBuild workspaces.");
            }
            std::cerr << "verify requires --project <root> or --workspace <runs task root>.\n";
            return 2;
        }
...
int AppMain(int argc, char** argv)
{
    ...
    analyze->add_option("--project", analyze_options.project, "Path to the target project root.");
    analyze->add_option("--solution", analyze_options.solution, "Legacy path to the target .sln file or project root directory.");
    ...
    auto* verify = app.add_subcommand("verify", "Verify a project profile or build a legacy workspace with MSBuild.");
    verify->add_option("--project", verify_options.project, "Path to the target project root.");
    verify->add_option("--workspace", verify_options.workspace, "Legacy workspace task root or repo directory.");
    ...
    auto* doctor = app.add_subcommand("doctor", "Inspect project profile, indexing, and verification readiness.");
    doctor->add_option("--project", doctor_options.project, "Path to the target project root.")->required();
    doctor->add_flag("--json", doctor_options.json, "Print a machine-readable JSON summary.");
    ...
    try
    {
        ...
        if (*doctor)
        {
            return RunDoctorCommand(doctor_options);
        }
