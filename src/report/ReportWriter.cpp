...
ReportWriteResult WriteReport(const ReportWriteRequest& request)
{
...

    {
        ...
        markdown << "- Workspace modified: "
                 << (redacted_diff_report.workspace_modified ? "true" : "false") << "\n\n";
        if (!request.audit.empty())
        {
            markdown << "## Project Profile Audit\n";
            markdown << "- Project kind: " << request.audit.value("project_kind", "unknown") << "\n";
            markdown << "- Profile source: " << request.audit.value("profile_source", "unknown") << "\n";
            markdown << "- Indexed files: " << request.audit.value("indexed_files_count", 0) << "\n";
            markdown << "- Skipped files: " << request.audit.value("skipped_files_count", 0) << "\n";
            markdown << "- Verify commands: " << request.audit.value("verify_commands_count", 0) << "\n\n";
        }
